...
def main():
    ...
    numQueries = [3000, 1000, 500, 100]
    budgets= [1000, 500, 200, 100, 50, 10]

    for nq in numQueries:
        for eps in budgets:
            print("\n\n**************** QUERIES:", nq, "; EPSILON: ", eps, "****************")

            params.maxQueries = nq
            params.name = mctsType + "_" + str(nq) + "_queries_Eps_" + str(eps)
            params.resultsFilename = "Results/Private/"+ dataset + "/" + mctsType + "_" + str(nq) + "_queries_Eps" + str(eps)
